...
- RLHF 
  ...
  - RLHF PPO
    ...
    - human preference alignment
      ...
      - the reward model r(prompt, generation) is trained in a supervised manner before the stage of RL fine-tuning, which is used to score the human preference of a generation for prompt
    ...
    - models in RLHF stage 3 
      ...
      - policy (actor)
        - initialized from the reference policy model

      - value (critic)
        - initialized from the reward model
        - trained by a temporal difference (TD) loss
  ...
  - DPO
    ...
    - the analytical form of the reward model can be derived from the learning objective of KL-penalized RLHF PPO
